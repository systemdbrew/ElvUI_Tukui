...
local function ApplyCommon()
	...
	for _, unit in ipairs({ "player", "target" }) do
		...
		f.enable = true; f.width = 250; f.height = 57; f.orientation = "LEFT"; f.smartAuraPosition = "DISABLED"
		local infoPanel = Ensure(f, "infoPanel"); infoPanel.enable = true; infoPanel.height = 21; infoPanel.transparent = false
		...
		local castbar = Ensure(f, "castbar"); castbar.enable = true; castbar.width = 250; castbar.height = 21; castbar.icon = true; castbar.iconAttached = false
		local raidicon = Ensure(f, "raidicon"); raidicon.attachTo = "TOP"; raidicon.attachToObject = "Health"; raidicon.xOffset = 0; raidicon.yOffset = 2
	end
	...
	local focusTarget = Ensure(units, "focustarget"); focusTarget.enable = true; focusTarget.width = 164; focusTarget.height = 20
	local np = Ensure(db, "nameplates"); np.statusbar = "ElvUI Norm"; np.font = "Expressway"; np.fontSize = 12; np.fontOutline = "OUTLINE"
	local npcUnits = Ensure(np, "units")
	for _, key in ipairs({ "FRIENDLY_NPC", "ENEMY_NPC" }) do local n = Ensure(npcUnits, key); n.width = 128; n.height = 14 end
end

local layouts = {
	desktop = {
		uiScale = 0.70,
		player = "BOTTOM,ElvUIParent,BOTTOM,-235,102", target = "BOTTOM,ElvUIParent,BOTTOM,235,102", targettarget = "BOTTOM,ElvUIParent,BOTTOM,0,102", pet = "BOTTOM,ElvUIParent,BOTTOM,0,176",
		focus = "BOTTOM,ElvUIParent,BOTTOM,-279,316", focustarget = "BOTTOM,ElvUIParent,BOTTOM,-279,341",
		bar1 = "BOTTOM,ElvUIParent,BOTTOM,0,12", bar2 = "BOTTOM,ElvUIParent,BOTTOM,-251,12", bar3 = "BOTTOM,ElvUIParent,BOTTOM,251,12",
		minimap = "TOPRIGHT,ElvUIParent,TOPRIGHT,-10,-10",
		experience = "BOTTOMLEFT,ElvUIParent,BOTTOMLEFT,5,235", honor = "BOTTOMRIGHT,ElvUIParent,BOTTOMRIGHT,-5,235",
	},
	laptop = {
		uiScale = 0.64,
		player = "BOTTOM,ElvUIParent,BOTTOM,-220,92", target = "BOTTOM,ElvUIParent,BOTTOM,220,92", targettarget = "BOTTOM,ElvUIParent,BOTTOM,0,92", pet = "BOTTOM,ElvUIParent,BOTTOM,0,160",
		focus = "BOTTOM,ElvUIParent,BOTTOM,-255,285", focustarget = "BOTTOM,ElvUIParent,BOTTOM,-255,308",
		bar1 = "BOTTOM,ElvUIParent,BOTTOM,0,10", bar2 = "BOTTOM,ElvUIParent,BOTTOM,-235,10", bar3 = "BOTTOM,ElvUIParent,BOTTOM,235,10",
...
function NS:ApplyProfile(preset)
	...
	SetMover("ElvUF_PlayerMover", layout.player); SetMover("ElvUF_TargetMover", layout.target); SetMover("ElvUF_TargetTargetMover", layout.targettarget); SetMover("ElvUF_PetMover", layout.pet)
	SetMover("ElvUF_FocusMover", layout.focus); SetMover("ElvUF_FocusTargetMover", layout.focustarget)
	SetMover("ElvAB_1", layout.bar1); SetMover("ElvAB_2", layout.bar2); SetMover("ElvAB_3", layout.bar3); SetMover("MinimapMover", layout.minimap)
	SetMover("LeftChatMover", "BOTTOMLEFT,ElvUIParent,BOTTOMLEFT,4,4"); SetMover("RightChatMover", "BOTTOMRIGHT,ElvUIParent,BOTTOMRIGHT,-4,4")
	SetMover("ExperienceBarMover", layout.experience); SetMover("HonorBarMover", layout.honor)
